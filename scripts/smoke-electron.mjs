...
assert.equal(result.localFontSizeApplied, true, "partial text selection must apply font size only to selected characters");
assert.equal(result.localTextColorApplied, true, "partial text selection must apply text color only to selected characters");
assert.equal(result.localHighlightApplied, true, "partial text selection must apply highlight only to selected characters");
assert.equal(result.unselectedRunsUnchanged, true, "unselected title/description runs must remain unchanged after local formatting");
assert.equal(result.textUndoWorked, true, "Ctrl/Cmd+Z inside text editing must undo text edits without touching canvas nodes");
assert.equal(result.textClipboardStayedLocal, true, "Ctrl/Cmd+C/X/V inside text editing must stay in the editor and preserve supported rich text");
assert.equal(result.imeSafe, true, "IME composition must not be submitted or cancelled by Enter/Esc while composing");
